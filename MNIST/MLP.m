...
max_iter = 1000;             % # iterate for so long
bias = 0;
...
y.h_out  = zeros(neurons_h,layers);    % what comes out of each layer
y.o_out  = zeros(neurons_out);         % final output neuron(s)
...
d.out = zeros(neurons_out);
d.h   = zeros(neurons_h,layers);
...
%     if iter > 10 && var(errors(end-4:end)) < eta*eta; converged = 1; end
    disp(['iteration ', num2str(iter), ', Err: ', num2str(total_err)])
    if mod(iter,10)==0
...
disp(['Total correct: ', sprintf('%.2f',corr) , '%, Runtime: ', sprintf('%.2f',endtime/60),' min'])
